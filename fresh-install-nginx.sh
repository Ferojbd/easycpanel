...
echo ""
sleep 1
log "========== Installing Engintron nGinx for cPanel & configuring / optimizing it =========="
sleep 2
echo ""
...
log "Installed and Enabled Engintron nGinx Web Server as Web Server Proxy for Apache."
sleep 2
# Path to the Nginx custom rules file
FILE_PATH="/etc/nginx/custom_rules"

# Uncomment the line and replace the placeholder IP address with the server's actual IP address
sed -i "/^# set \$PROXY_DOMAIN_OR_IP/c\set \$PROXY_DOMAIN_OR_IP \"${serverip}\"; # Use your cPanel's shared IP address here" "$FILE_PATH"

# Add a new line at the end of the file to disable micro-caching
echo 'set $CACHE_BYPASS_FOR_DYNAMIC 1; # Disables micro-caching' >> "$FILE_PATH"

# Restart nGinx and Apache Web Service to apply changes.
bash /engintron.sh res ;
log "Enabled Cloudflare compatibility with nGinx Web Server."
sleep 2
log "Engintron is installed and configured"
sleep 2
echo ""
echo ""
log "###########################################################################"
